...
const Footer = () => (
  <section className={`${styles.flexCenter} ${styles.paddingY} flex-col`}>
          ...
          >
            ...
            <ul className="list-none mt-4">
              {footerLink.links.map((link, index) => (
                <li
                  key={link.name}
                  className={`font-poppins font-light text-base leading-[24px] text-dimWhite hover:text-secondary cursor-pointer ${index !== footerLink.links.length -1 ? 'mb-4' : 'mb-0'}`}
                >
                  {link.name}
                </li>
              ))}
    ...
    <div className="w-full flex justify-between items-center md:flex-row flex-col pt-6 border-t border-t-[#3f3e45]">
      <p className="font-poppins font-normal text-center  text-lg leading-[27px] text-white">2023 HooBank. All Rights Reserved.</p>
